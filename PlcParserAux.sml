...
fun makeType (args: (plcType * string) list) : plcType =
  case args of
    [] => ListT []
  | (t, _) :: tail =>
      let val ListT typelist = makeType tail
      in ListT (t :: typelist)
      end;

(* Create a function expression. *)
fun makeFun
  (f: string, xs: (plcType * string) list, rt: plcType, e1: expr, e2: expr) :
  expr =
  case xs of
...
fun makeAnon (xs: (plcType * string) list, e: expr) : expr =
  ...
  | (t, x) :: [] => Anon (t, x, e)
  | _ =>
      let val t = makeType xs
      in let val e' = makeFunAux (1, xs, e) in Anon (t, "$list", e') end
      end;
